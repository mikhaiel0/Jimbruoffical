...
Jimbru.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram", dontAddCommandList: true }, async (message, match) => { 
    const { status, type, data } = await insta(match[1])
    if (!status) return await message.sendMessage('_Download failed_')
    await message.client.sendMessage(message.jid, LOAD_ING, MessageType.text, { quoted: message.data });
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: Config.JINN, quoted: message.data })
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: Config.JINN, quoted: message.data })
});
    //Credit: Farhan dqz
