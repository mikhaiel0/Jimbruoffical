...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to jimbru voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Jimbroottan.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {

    if (message.message.startsWith('Jimbru') && conf.TALKING_JINN !== 'true') {        
        ...
        var finm = message.message.replace('jimbru', '').replace(' ', '')   
...
Jimbroottan.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {

        if (conf.TALKING_JINN == 'true' && ((!message.jid.includes('g.us')) || (message.jid.includes('g.us') && 
...
Jimbroottan.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
if (conf.LANG == 'TR') {
    ...
    fulleva_dsc = 'Tam fonksiyonel Jimbroottan özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'

    already_on = 'Jimbroottan yapay zekası halihazırda tüm fonksiyonları etkin.'

    already_off = 'Jimbroottan yapay zekası halihazırda yarı fonksiyonel çalışıyor.'

    succ_on = 'Jimbroottan, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'

    succ_off = 'Jimbroottan, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'

}

if (conf.LANG == 'EN') {

    fulleva_dsc = 'Activates full functional Jimbroottan features. Turn your account into a ai chatbot!'

    already_on = 'Jimbroottan artificial intelligence is already fully functional.'

    already_off = 'Jimbroottan artificial intelligence is currently running semi-functional.'

    succ_on = 'Jimbroottan Opened Fully Functionally! Please wait a bit! ✅'

    succ_off = 'Jimbroottan Set to Semi-Functional! Please wait a bit! ☑️'

}

if (conf.LANG == 'ML') {

    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ ജിംബ്രൂട്ടൻ സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'

    already_on = 'ജിംബ്രു കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'

    already_off = 'ജിംബ്രു AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'

    succ_on = 'ഞാൻ പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'

    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി ജിംബ്രുട്ടൻ സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'

}

Jimbroottan.addCommand({ pattern: 'jimbru ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.jimbru on / off' }, (async (message, match) => {

    var jinnh_status = `${conf.TALKING_JINN}`

    if (match[1] == 'on') {

        if (jinnh_status == 'true') {
